...
contract Sender is Ownable {
    ...
    IRouterClient private constant ROUTER =
    ...
    error Sender__InsufficientBalance(
        IERC20 token,
    ...
    function transferTokens(
        address _receiver,
        uint256 _amount,
        address _target //Vault contract on the destination chain
    ) external returns (bytes32 messageId) {
        if (_amount > USDC_TOKEN.balanceOf(msg.sender)) {
            revert Sender__InsufficientBalance(
                USDC_TOKEN,
