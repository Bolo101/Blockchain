...
contract OrderTracker{
    ...
    modifier isOwner(){
        require(msg.sender == owner,"Not the owner");//the deployer of the contract has privileges
        _;
    }
    ...
    function submitOrder(string memory _product,uint _quantity,address _recipient) public isOwner{
    ...
    function updateOrderSending(uint _orderId) public isOwner{
        Order[_orderId]._etape = etape.sending;//change order status to "sending"
        
    }
    function updateOrderDelivered(uint _orderId) public isOwner{
