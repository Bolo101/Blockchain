...
contract FlatManagement is Owner {
    ...
    function getProperties(
        address _owner
    ) public view isOwner returns (property[] memory) {
        //returns all data for a specific address
    ...
    function getPropertiesByName(
        ...
        for (uint i = 0; i < properties.length; i++) {
            if (
                keccak256(bytes(properties[i].name)) == keccak256(bytes(_name))
            ) {
                //can not check strings with == ; can not compare string with == operator, byte error returned so converted data in bytes before hash function to check the output
                return properties[i];
            }
        }
        revert("Unknown ID");
